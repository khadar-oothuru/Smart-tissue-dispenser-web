...
class AdminService {
  ...
  async fetchUsers() {
    try {
      ...
      const response = await axios.get(`${this.baseURL}/users/admin/users/`, {
  ...
  async fetchAdminStats() {
    try {
      ...
      const response = await axios.get(`${this.baseURL}/users/admin/stats/`, {
        headers: this.getAuthHeaders(),
      });
  ...
  async fetchAdminProfile() {
    try {
      ...
      const response = await axios.get(`${this.baseURL}/users/admin/profile/`, {
        headers: this.getAuthHeaders(),
      });
      console.log(
  ...
  async updateUserStatus(userId, isActive) {
    try {
      ...
      const response = await axios.patch(
        `${this.baseURL}/users/admin/users/${userId}/`,
        { is_active: isActive },
        {
          headers: this.getAuthHeaders(),
        }
  ...
  async createUser(userData) {
    try {
      ...
      const response = await axios.post(
        `${this.baseURL}/users/admin/users/`,
        userData,
        {
          headers: this.getAuthHeaders(),
        }
  ...
  async fetchAppLogs(page = 1, limit = 50) {
    try {
      ...
      const response = await axios.get(
        `${this.baseURL}/users/admin/logs/?page=${page}&limit=${limit}`,
        {
          headers: this.getAuthHeaders(),
        }
      );
  ...
  async updateAdminProfile(profileData) {
    try {
      ...
      const response = await axios.patch(
        `${this.baseURL}/users/admin/profile/`,
        profileData,
        {
          headers: this.getAuthHeaders(),
        }
